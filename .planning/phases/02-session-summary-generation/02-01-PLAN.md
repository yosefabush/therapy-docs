...
Output: A `src/lib/ai/` directory with summary generation logic. This plan exports only the low-level `generateAISummary(systemPrompt, userPrompt)` function. The higher-level convenience function that combines prompt building with generation will be added in Plan 02.
</objective>
